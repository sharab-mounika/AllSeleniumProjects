...
//		
		...
		// dynamic dropdowns we will be using click only
		Thread.sleep(2000);
...
//		
		...
		//System.out.println(driver.findElement(By.name("ctl00$mainContent$view_date2")).isEnabled());	// this will not work sometimes even though it is not enabled it will show true
	...
	// note: 1) to format or align entire code use ctrl+shift+f 
	//2) class name should start with cap and var name should start small and use camel case 
	//3) if you want to debug from certain point add toggel breakpoints at that line and run as debug execution will run asusual and ask for debug at that point from that point you can debug each and every step buy using step over and can use resume option to execute normally 
